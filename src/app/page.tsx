...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    recipes.forEach((rec) => {
      r[rec.id] = getAverageRating(rec.id);
    });
    setRatings(r);
  }, []);
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
      ...
      <section className="text-center mb-14">
        ...
        <p className="text-lg max-w-xl mx-auto leading-relaxed mb-8" style={{ color: "var(--ink-light)" }}>
          A curated collection of authentic Italian gelato recipes — from
          sun-drenched Sicily to the hills of Piedmont.
        </p>
        ...
        <form onSubmit={handleSearch} className="flex gap-2 max-w-md mx-auto">
          <input
            ...
            placeholder="Search recipes, flavors, ingredients…"
            className="museum-input"
          />
          <button type="submit" className="museum-btn-primary whitespace-nowrap">
            Search
          </button>
        </form>
      </section>
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          <h2 className="section-title">
            {activeTag ? `#${activeTag}` : "Featured Flavors"}
          </h2>
          <a
            href="/browse"
        ...
        {displayedRecipes.length === 0 ? (
          <div className="text-center py-20">
            ...
            <button
              type="button"
              onClick={() => setActiveTag(null)}
              className="museum-btn-outline mt-4"
            >
              Clear filter
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayedRecipes.map((recipe) => (
              <RecipeCard
                key={recipe.id}
                recipe={recipe}
                averageRating={ratings[recipe.id]}
              />
            ))}
          </div>
        )}
      ...
      <section className="mt-16 text-center py-12 rounded-3xl" style={{ backgroundColor: "var(--mint-light)", border: "1px solid var(--border-mint)" }}>
        <p className="font-serif text-2xl font-semibold mb-2" style={{ color: "var(--ink)" }}>
          Not sure what to make?
        </p>
        <p className="text-sm mb-6" style={{ color: "var(--ink-light)" }}>
          Describe your cravings and let our AI Flavor Finder guide you.
        </p>
        <a href="/ai-finder" className="museum-btn-primary inline-block">
          Try the AI Finder →
        </a>
      </section>
    </div>
