...
export default function V2HomePage() {
  ...
  function handleSearch(e: React.FormEvent) {
    e.preventDefault();
    if (search.trim()) router.push(`/v2/search?q=${encodeURIComponent(search.trim())}`);
  }
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
      ...
      <section>
        <div className="flex items-center justify-between mb-6">
          ...
          <a href="/v2/browse" className="text-sm font-medium" style={{ color: "var(--mint-dark)" }}>View all →</a>
        </div>
      ...
      <section className="mt-16 text-center py-12 rounded-3xl" style={{ backgroundColor: "var(--mint-light)", border: "1px solid var(--border-mint)" }}>
        ...
        <a href="/v2/ai-finder" className="museum-btn-primary inline-block">Try the AI Finder →</a>
      </section>
    </div>
  );
}
