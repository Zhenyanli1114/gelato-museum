    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <p className="text-sm mb-6" style={{ color: "var(--ink-muted)" }}>
          We couldn&apos;t find a gelato with that name.
        </p>
        <Link href="/browse" className="museum-btn-primary inline-block">
          Browse All Recipes
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-10">
        ...
        <div className="relative aspect-square rounded-2xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
          <Image
            ...
            priority
            onError={(e) => {
              (e.target as HTMLImageElement).src = `https://placehold.co/600x600/ccfbef/0d9488?text=${encodeURIComponent(recipe.name)}`;
            }}
          />
        </div>
        ...
        <div className="flex flex-col justify-between">
          <div>
            ...
            <h1 className="font-serif text-4xl font-bold leading-tight mb-3" style={{ color: "var(--ink)" }}>
              {recipe.name}
            </h1>
            ...
            <div className="flex flex-wrap gap-4 mb-4">
              ...
              <div className="flex flex-col">
                ...
                <span className="font-medium text-sm" style={{ color: difficultyColor[recipe.difficulty] }}>
                  {recipe.difficulty}
                </span>
              </div>
          ...
          <div className="flex gap-3 flex-wrap">
            <button
              type="button"
              onClick={isClient ? handleFavorite : undefined}
              className="museum-btn-outline flex items-center gap-2"
              style={
                favorited
                  ? { borderColor: "#e11d48", color: "#e11d48", backgroundColor: "#fff1f2" }
                  : {}
              }
            >
            ...
            <Link
              href={`/recipe/${id}/reviews`}
              className="museum-btn-primary flex items-center gap-2"
            >
              ★ Reviews
            </Link>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {/* Ingredients */}
        <section>
          <h2 className="font-serif text-2xl font-semibold mb-4" style={{ color: "var(--ink)" }}>
            Ingredients
          </h2>
          <ul className="space-y-2">
            {recipe.ingredients.map((ing, i) => (
              <li
                ...
                style={{ borderBottom: "1px solid var(--border)", color: "var(--ink-light)" }}
              >
                <span style={{ color: "var(--mint-dark)", fontWeight: "600", flexShrink: 0 }}>
          ...
          <ol className="space-y-4">
            {recipe.steps.map((step, i) => (
              <li key={i} className="flex gap-3">
                ...
                <p className="text-sm leading-relaxed pt-0.5" style={{ color: "var(--ink-light)" }}>
                  {step}
                </p>
              </li>
            ))}
      ...
      {/* Link to Reviews */}
      <div
        className="mt-10 p-6 rounded-2xl flex items-center justify-between flex-wrap gap-4"
      ...
      >
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: "var(--ink-light)" }}>
            Share your experience and help others discover great gelato.
          </p>
        </div>
        <Link href={`/recipe/${id}/reviews`} className="museum-btn-primary">
          Write a Review →
        </Link>
      </div>
    </div>
